		...
		<main className="min-h-screen bg-white">
			...
			<section className="pt-24 md:pt-24 pb-16 px-6 md:px-12 max-w-[1600px] mx-auto flex flex-col md:flex-row gap-10 md:gap-16">
				...
				<div className="w-full md:w-[60%] flex flex-col md:flex-row-reverse gap-4 md:gap-6 relative">
					...
					{/* 1. GAMBAR UTAMA (Rasio 1:1 Kotak Sempurna) */}
					...
					{/* 2. THUMBNAILS / PREVIEW (Rasio 1:1 Kotak Sempurna) */}
					<div className="flex flex-row md:flex-col gap-2 md:gap-3 z-30 absolute top-4 left-4 md:static md:w-[15%] md:pt-2">
						{product.images.map((img, idx) => (
							<button
								...
								className={`aspect-square w-12 md:w-full relative overflow-hidden bg-gray-100 border-2 transition-all duration-200 flex-shrink-0 ${currentMainImageIndex === idx ? 'border-gray-900' : 'border-gray-200'}`}
							>
								<Image src={img} alt={`Preview ${idx + 1}`} fill className="object-cover object-center" />
							</button>
						))}
					</div>
				</div>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-2">
								{product.sizes.map((item) => (
									<button
										...
										className={`py-3 flex flex-col items-center justify-center border transition-all duration-200 ${!item.available
									...
									>
										<span className="text-xs font-bold uppercase">{item.size}</span>
										<span className={`text-[8px] mt-1 tracking-[0.15em] uppercase font-bold ${!item.available ? 'text-gray-400' : selectedSize === item.size ? 'text-gray-300' : 'text-green-600'
											}`}>
