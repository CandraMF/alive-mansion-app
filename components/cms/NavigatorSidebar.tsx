...
interface NavigatorSidebarProps {
  ...
  pageData: any;
  setPageData: React.Dispatch<React.SetStateAction<any>>; // Prop baru
  activeItem: any;
...
export default function NavigatorSidebar({
  ...
  const handleDragOver = (e: React.DragEvent, targetId: string, targetType: 'section' | 'block', isContainer: boolean) => {
    ...
    // Logika pembatasan: Block tidak bisa ditaruh di luar section
    if (dragState.type === 'block' && targetType === 'section') {
      setDropState({ id: targetId, position: 'inside' });
      return;
    }

    // Hitung posisi kursor relatif terhadap elemen yang ditarget
    ...
    else position = isContainer ? 'inside' : 'after'; // Jika bukan container, default ke after
  ...
  const handleDrop = (e: React.DragEvent, targetId: string, targetType: 'section' | 'block') => {
    ...
    // EKSEKUSI PEMINDAHAN DATA (MUTASI JSON)
    setPageData((prev: any) => {
      const newData = JSON.parse(JSON.stringify(prev)); // Deep clone
      let extractedNode: any = null;

      // 1. Fungsi Pencabut Node
      ...
      // 2. Fungsi Validasi Anak-ke-Induk (Mencegah blackhole / infinite loop)
    ...
    return (
      <div key={block.id} className="w-full">
        <div
          ...
          className={cn(
            "flex items-center gap-1.5 py-1.5 px-2 rounded-md cursor-grab transition-all text-xs select-none border border-transparent relative",
            isSelected ? "bg-blue-100 text-blue-700 font-bold" : "hover:bg-gray-100 text-gray-700",
            dragState?.id === block.id && "opacity-30",
            // INDIKATOR DROP VISUAL
            ...
            return (
              <div key={section.id} className="w-full">
                <div
                  ...
                  className={cn(
                    "flex items-center gap-1.5 py-1.5 px-2 rounded-md cursor-grab transition-all text-xs border border-transparent",
